...
webp                    = require('gulp-webp'), // Images to WebP
...
gulp.task('build:imageWebP', function () {
    return gulp.src(path.src.img) 
        .pipe(webp())
        .pipe(imagemin({
            use: [
...
gulp.task('buildProd:imageWebP', function () {
    return gulp.src(path.src.img) 
        .pipe(webp())
        .pipe(imagemin({
            use: [
...
gulp.task('build:dev', gulp.series('clean:build', gulp.parallel(
    'build:html','build:js','copy:js','build:css','copy:css','build:fonts','build:image', 'build:imageWebP', 'copy_to_root:dev'
    )));
